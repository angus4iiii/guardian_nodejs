...
async function runSequence(crankSerial) {
  ...
  // crankside
  try {
    const csResp = await fetch(`${BACKEND_URL}/api/crankside?productionsn=${encodeURIComponent(crankSerial)}`);
    if (csResp.ok) {
  ...
  try {
    const podoqcResponse = await fetch(`${BACKEND_URL}/api/podoqc`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ crankSerial })
    });
  ...
  try {
    const response = await fetch(`${BACKEND_URL}/api/autocal`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ crankSerialNumber: crankSerial })
    });
    if (response.ok) {
      const data = await response.json();
      if (data.resultcodes && data.resultcodes.length > 0) {
        newResults[1] = data.resultcodes[0] === 0 ? 0 : 1;
        newResults[2] = data.variationmetric && data.variationmetric[0] !== undefined && vmThresholdLocal !== null
          ? (data.variationmetric[0] <= vmThresholdLocal ? 0 : 1)
          : 2;
      } else {
        newResults[1] = 1; newResults[2] = 1;
      }
    } else {
      newResults[1] = 1; newResults[2] = 1;
    }
  ...
  try {
    const oqcResponse = await fetch(`${BACKEND_URL}/api/progressevents`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productionsn: crankSerial })
    });
    if (oqcResponse.ok) {
  ...
  if (String(crankSideValue) !== '1') {
    try {
      ...
      if (fmResponse.ok) {
        ...
        if (fmData.events && fmData.events.some(ev => typeof ev === 'string' && ev.includes('FindMy PASS'))) {
          newResults[6] = 0;
        } else {
          newResults[6] = 1;
        }
